...
{
	snprintf(ErrorBuf+strlen(ErrorBuf), sizeof(ErrorBuf)-strlen(ErrorBuf)-1, ": [err=%u] ", LastErr);
    PICMLX_ErrorCodeToMessage(LastErr, ErrorBuf+strlen(ErrorBuf),sizeof(ErrorBuf)-strlen(ErrorBuf)-1);
    fprintf(stderr,"%s\n",ErrorBuf);
}
...
{
	snprintf(ErrorBuf+strlen(ErrorBuf), sizeof(ErrorBuf)-strlen(ErrorBuf)-1, ": [err=%u] ", LastErr);
    PIPLX_ErrorCodeToMessage(LastErr, ErrorBuf+strlen(ErrorBuf),sizeof(ErrorBuf)-strlen(ErrorBuf)-1);
    fprintf(stderr,"%s\n",ErrorBuf);
}
...
int main(int argc, char **argv)
{
    ...
    if (LastErr != NO_ERROR){
    	snprintf(ErrorBuf, sizeof(ErrorBuf), "Error connecting to LXI on '%s:%u'", LxiIp, Port);
    ...
    if (LastErr != NO_ERROR){
    	  snprintf(ErrorBuf, sizeof(ErrorBuf), "Error on PIPLX_OpenSpecifiedCard(Sid=%ld, Bus=%u, Slot=%u, &CardNum=%p)",
    ...
    if (LastErr != NO_ERROR){
    	  snprintf(ErrorBuf, sizeof(ErrorBuf), "Error on PIPLX_CardId(Sid=%ld, CardNum=%u, &Str=%p,StrLen=%zu)",
...
exit3:
  LastErr = PIPLX_CloseSpecifiedCard(Sid,CardNum);
  if (LastErr != NO_ERROR){
	  ret = EXIT_FAILURE;
  	  snprintf(ErrorBuf, sizeof(ErrorBuf), "Error on PIPLX_CloseSpecifiedCard(Sid=%ld, CardNum=%u)",
  			  Sid,CardNum);
  	  AppendAndPrintPiplxError();
  }
  CardNum = 0;
...
exit2:
  ...
  if (LastErr != NO_ERROR){
	  ret = EXIT_FAILURE;
	  snprintf(ErrorBuf, sizeof(ErrorBuf), "Error on PICMLX_Disconnect(Sid=%ld)", Sid);
	  AppendAndPrintPicmlxError();
  }
  Sid = INVALID_SESSION;
